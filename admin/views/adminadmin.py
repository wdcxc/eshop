    ...
    def updateGroup(self, request):
        ...
        if request.method == "GET":
            ...
            group.__dict__.update(
                {"nodeIds": ",".join([str(node["id"]) for node in group.nodes.all().values()]), "nodes": nodes})
    ...
    def setting(self, request):
        ...
        if request.method == "GET":
            ...
            if newPassword != confirmPassword:
                self.context = {"code": 4, "msg": "新密码和确认密码不一致", "data": {}}
            else:
                oldPassword = hashlib.sha512(oldPassword.encode("utf-8")).hexdigest()
                if oldPassword != admin.password:
                    self.context = {"code": 4, "msg": "旧密码错误", "data": {}}
                else:
                    admin.password = hashlib.sha512(newPassword.encode("utf-8")).hexdigest()
                    admin.save()
                    self.context = {"code": 200, "msg": "修改密码成功", "data": {}}
