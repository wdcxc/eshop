...
class AppAdminView(AdminBaseView):
    ...
    def addCarousel(self, request):
        ...
        if request.method == 'POST':
            ...
            # todo:数据清洗
            newCarousel = CarouselModel(title=carousel["title"], show=carousel["show"], order=carousel["order"],
                                        imgUrl=carousel["imgUrl"], linkUrl=carousel["linkUrl"])
            newCarousel.save()
    ...
    def updateCarousel(self, request):
        ...
        if request.method == "GET":
            ...
            for carouselKey in carouselKeys:
                carousel[carouselKey] = request.POST.get(carouselKey)
            print(carousel)
            CarouselModel.objects.filter(id=carousel["id"]).update(title=carousel["title"], show=carousel["show"],
                                                                   order=carousel["order"], imgUrl=carousel["imgUrl"],
    ...
    def addShoppingGuideChannel(self, request):
        ...
        if request.method == "GET":
            ...
            newChannel = ShoppingGuideChannel(name=channel["name"], show=channel["show"], order=channel["order"],
    ...
    def addShoppingGuideSubchannel(self, request):
        if request.method == "GET":
            ...
            newSubchannel = ShoppingGuideSubchannel(parentId=subChannel["parentId"], name=subChannel["name"],
                                                    show=subChannel["show"], order=subChannel["order"],
                                                    addAdminId=subChannel["addAdminId"])
            newSubchannel.save()
    ...
    def addShoppingGuideProduct(self, request):
        ...
        if request.method == "GET":
            ...
            print(vals)
            product = ShoppingGuideProduct(name=vals["name"], parentId=vals["parentId"], linkUrl=vals["linkUrl"],
                                           productImgUrl=vals["productImgUrl"], show=vals["show"], order=vals["order"],
                                           description=vals["description"], addAdminId=vals["addAdminId"])
            product.save()
    ...
    def _sortProductCategories(self, productCategories):
            ...
            for i, productCategory in enumerate(productCategories):
                if productCategory["grade"] == curGrade:
                    ...
                    if productCategory["parentId"] in sortedProductCategoriesDict:
                        sortedProductCategoriesDict[productCategory["parentId"]]["subCategories"].append(
                            productCategory)
                    else:
        ...
        """
        productCategories = list(productCategories.values())
        sortedProductCategoriesDict = OrderedDict()
        while productCategories:
            curGrade = productCategories[0]["grade"]
            for i, productCategory in enumerate(productCategories):
                if productCategory["grade"] == curGrade:
